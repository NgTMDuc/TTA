    ...
    with h5py.File(hdf5_file, 'w') as hf:
        with open(meta_path, 'r') as f:
            ...
            for line in tqdm(rdr):
                ...
                hf.create_dataset(h5py_path, data=np.array(x))
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join(dset_dir, 'waterbirds_pretrained_model_update.pickle')):
        ...
        with torch.no_grad():
            ...
            net.eval()
            with open(file=os.path.join(dset_dir, 'waterbirds_pretrained_model_update.pickle'), mode='wb') as f:
                pickle.dump(net, f)
            test_waterbirds(net, test_loader, epochs)
    else:
