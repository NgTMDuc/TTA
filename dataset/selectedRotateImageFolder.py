...
class SelectedRotateImageFolder(datasets.ImageFolder):
    ...
    def set_target_class_dataset(self, target_class_index, logger=None):
        ...
        self.samples = [(path, idx) for (path, idx)
                        in self.original_samples if idx in self.target_class_index]
...
def prepare_train_dataset(args, use_transforms=True):
    ...
    trset = SelectedRotateImageFolder(traindir, tr_transforms_local,
                                      original=True, rotation=args.rotation,
...
def prepare_test_data(args, use_transforms=True):	
    ...
    if not hasattr(args, 'workers'):
        args.workers = 1
    teloader = torch.utils.data.DataLoader(teset, batch_size=args.test_batch_size,
