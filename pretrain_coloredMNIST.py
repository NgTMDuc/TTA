...
def test_model(model, device, test_loader, set_name="test set"):
    ...
    with torch.no_grad():
        for data, target, color in tqdm(test_loader):
...
def erm_train(model, device, train_loader, optimizer, epoch, args):
    ...
    model.train()
    CELoss = torch.nn.CrossEntropyLoss()
    for batch_idx, (data, target, group) in enumerate(tqdm(train_loader)):
...
def train_and_test_erm(model, args):
    ...
    test_loader = torch.utils.data.DataLoader(
      ColoredMNIST(root=args.root_dir, env='test',
                   flip=True,
                   transform=transforms.Compose([
                       transforms.ToTensor(),
                       transforms.Normalize((0.1307, 0.1307, 0.), (0.3081, 0.3081, 0.3081))
    ...
    if not os.path.exists(os.path.join(args.root_dir, 'ColoredMNIST_model_update.pickle')):
...
if __name__=='__main__':    
    ...
    if not os.path.exists(os.path.join(args.root_dir, 'ColoredMNIST_model_update.pickle')):
        with open(file=os.path.join(args.root_dir, 'ColoredMNIST_model_update.pickle'), mode='wb') as f:
            pickle.dump(model, f)
    else:
